...
void testDeleteDogS()
{
	...
	assert(service.getAllDogsService().getSize() == 1);
}
...
void testAdoptedS()
{
	...
	assert(service.getAdoptedDogsService().getSize() == 2);
...
void testResize()
{
	...
	assert(service.getAllDogsService().getSize() == 4);
}
...
void testFilterDogs()
{
	...
	DynamicVector<Dog> filteredDogs1 = service.filterBreedAgeService("b", 3);
	assert(filteredDogs1.getSize() == 3);

	DynamicVector<Dog> filteredDogs = service.filterBreedAgeService("a", 2);
	std::cout<<filteredDogs.getSize();
	assert(filteredDogs.getSize() == 1);
}
