...
{
	//prints all dogs
	DynamicVector<Dog> d = service.getAllDogsService();
...
DynamicVector<Dog> UI::seeAdoptionList()
{
...
void UI::printAllAdoptedDogs(Service service)
{
	...
	DynamicVector<Dog> d = seeAdoptionList();
	for (int i = 0; i < d.getSize(); i++)
		printDog(d[i]);
}
...
void UI::printBreedAge()
{
	...
	DynamicVector<Dog> d = service.filterBreedAgeService(breed, age);
	for (int i = 0; i < d.getSize(); i++)
		printDog(d[i]);
}
