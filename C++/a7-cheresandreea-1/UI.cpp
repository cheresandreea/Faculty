...
//void UI:: testWrite()
//{
//	ofstream f("Dogs.html", ios::app);
//	if (!f.is_open())
//		return;
//
//	vector<Dog> dogs = { Dog{"ShihTzu", "Cris", 2, "link1"}, Dog{"Poodle", "Mera", 1, "link2"},
//	Dog{"Bulldog", "Rex", 3, "link3"}, Dog{"Pitbull", "Nero", 5, "link4"} };
//
//	for (auto d : dogs)
//	{
//		f << d;
...
void UI::updateDogUI()
{
	...
	try {
		...
		else
		{
			...
			Dog d;
			try {
				Dog d = Dog(newBreed, newName, int(newAge), newLink);
				PetValidator::validate(d);
			}
			catch (ValidationException& e) {
				std::cout << e.what() << std::endl;
...
void UI::openUser()
{
	...
	while (true)
	{
		...
		std::cout << "4. Save dogs to the file" << endl;
		std::cout << "5. Exit" << std::endl;
		std::cin>>op;
		switch (op)
		{
		...
		case 3:
			...
			break;
		case 4:
			service.saveAdoptedDogsToSpecificFile();
			break;
		case 5:
			return;
		}
